...
\usepackage{fontspec}  % 使用 fontspec 来指定字体
\usepackage{xeCJK}     % 使用 xeCJK 支持中文
\setCJKmainfont{PingFang SC} % 设置中文字体，例如宋体
...
% \titlebackground*{images/ust_bg.png}
...
\author[H. Tan at al.]{\href{mailto:hr.tan@connect.ust.hk}{Huiri Tan}}
\institute[HKUST]{The Hong Kong University of Science and Technology}

\date{\today}

\setbgcolor{expowhite} % set background color back to default light color
\setconflogo{images/conference.pdf}
...
% \footlinecolor{ustblue}

\maketoc[hideallsubsections]
...
\begin{chapter}[images/ust_scratch.jpg]{ustblue}{Slide Layouts}
...
\begin{chapter}[images/ust_wallpaper.jpg]{ustblue}{Personalization}
\textit{Themes, fonts and colors}
\end{chapter}

\begin{frame}[fragile]{Showing TOC}
You can show the table of contents at any point in your presentation using the \verb|\maketoc| command.
You can also choose to show/hide subsections toc using \verb|\setshowsectiontoc| command.
\end{frame}

\begin{frame}[fragile]{Author Information on Sidebar}
If you do not want author and institute information to be shown on the left sidebar throughout the presentation,
you can call the command \verb|\disableauthorsideinfo| in the preamble (i.e. before \verb|\begin{document}|).
\end{frame}
...
\begin{itemize}
    ...
    \item You can use \verb|\setbgcolor{color}| to change the light background color, e.g. \verb|\setbgcolor{ustyellow!20}|.
\end{itemize}
...
\begin{chapter}[images/ust_wallpaper.jpg]{ustblue}{Title goes here}
\textit{Subtitle goes here}
\end{chapter}

\themecolor{lightnologo}

\begin{chapter}[images/ust_wallpaper.jpg]{steelgray}{Title goes here}
\textit{Subtitle goes here}
\end{chapter}
\themecolor{light}
